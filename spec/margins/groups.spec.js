...
describe('Margins Groups List Page', function() {
    ...
    it('Page Subtitle should be \'Margins Groups\'', function(done) {
        page
            .getSubTitle()
            .then(function(title) {
                expect(title.toLowerCase()).toContain(i18n('marginsGroups'));
            })
            .then(done);
    }, 10000);
    ...
    it('Should inform when user tries to delete a group without any rules assigned', function(done) {
        destination
            ...
            .then(modal.getText)
            .then(function(text) {
                expect(text).toContain(i18n('marginsGroupDeleteNotification'));
            })
            .then(done)
    }, 10000);
